...
class DBManager:
    ...
    def get_vacancies_with_highest_salary(self):
        ...
        result = self.cursor.fetchall()
        for item in result:
            print(colorama.Fore.GREEN + f"ID_компании: {item[0]}, Вакансия: {item[1]}, " + colorama.Fore.GREEN +
                  f"{self.format_salary(item)}, " + colorama.Fore.RESET + f"Ссылка на вакансию: {item[4]}")
    ...
    def get_vacancies_with_keyword(self, query):
        ...
        for word in query.split():
            ...
            result = self.cursor.fetchall()
            if result:
                print(colorama.Fore.RED + f'Результаты по ключевому слову "{word}":' + colorama.Fore.RESET)
            for item in result:
